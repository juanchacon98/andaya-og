...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { reservation_id }: ReservationRequestedPayload = await req.json();

    console.log("Processing reservation-requested for:", reservation_id);
    ...
    const emailHtml = `
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">Nueva Solicitud de Reserva</h1>
        </div>
        
        <div style="background: white; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
          <p style="font-size: 16px; margin-bottom: 20px;">Hola ${owner.first_name},</p>
          
          <p style="font-size: 16px; margin-bottom: 20px;">Tienes una nueva solicitud de reserva para tu vehículo:</p>
          ...
          <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <div style="margin: 15px 0;">
              <strong style="color: #666;">Solicitante:</strong><br>
              <span style="font-size: 16px;">${reservation.renter.first_name} ${reservation.renter.last_name}</span>
            </div>
            ...
            <div style="margin: 15px 0; padding-top: 15px; border-top: 2px solid #e0e0e0;">
              ...
              <span style="font-size: 24px; color: #667eea; font-weight: bold;">Bs ${totalBs}</span>
            </div>
          </div>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${Deno.env.get("SUPABASE_URL")?.replace('.supabase.co', '.lovable.app') || ''}/owner/reservas/${reservation_id}?action=approve" 
            ...
            <a href="${Deno.env.get("SUPABASE_URL")?.replace('.supabase.co', '.lovable.app') || ''}/owner/reservas/${reservation_id}?action=reject" 
               style="display: inline-block; background: #ef4444; color: white; padding: 14px 32px; text-decoration: none; border-radius: 6px; font-weight: 600; margin: 5px; font-size: 16px;">
              ✗ Rechazar
            </a>
          </div>
          
          <p style="color: #666; font-size: 14px; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e0e0e0;">
            Este correo fue enviado por AndaYa. Si no esperabas esta notificación, puedes ignorarla.
          </p>
        </div>
      </body>
      </html>
    ...
    const emailResponse = await resend.emails.send({
      ...
      to: [owner.email],
      subject: `Nueva solicitud — ${reservation.vehicle.brand} ${reservation.vehicle.model}`,
      html: emailHtml,
    });
    ...
    console.error("Error in reservation-requested function:", error);
    return new Response(
