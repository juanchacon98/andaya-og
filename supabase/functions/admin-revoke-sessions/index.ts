...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Sesiones revocadas para usuario ${user_id}`);

    return new Response(
      JSON.stringify({ success: true, message: 'Todas las sesiones han sido revocadas' }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error: any) {
    console.error('Error en admin-revoke-sessions:', error);
    return new Response(
