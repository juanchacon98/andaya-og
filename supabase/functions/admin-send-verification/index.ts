...
serve(async (req) => {
  ...
  try {
    ...
    // Enviar email de verificación (reenvía invitación)
    ...
    console.log(`Email de verificación enviado a ${userData.user.email}`);

    return new Response(
      JSON.stringify({ success: true, message: 'Email de verificación enviado' }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error: any) {
    console.error('Error en admin-send-verification:', error);
    return new Response(
